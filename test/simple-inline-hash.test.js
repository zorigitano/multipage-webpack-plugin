...
const simpleConfig = require('../examples/simple-inline-hash/webpack.config.js');
const fs = testFs; // Use shared memoryfs instance

const simpleExamplePath = path.resolve(__dirname, '../examples/simple-inline-hash');
...
test('it should have the webpack bootstrap in a separate chunk [inline.chunk.js]', async t => {
  ...
  t.true(fsStats.includes('bananas.chunk.js'));
});
  ...
  t.true(
    getEntryKeysFromStats(stats).every(async (entryName) => {
      ...
      return templateContent
        .toString()
        .match(`<script type="text/javascript" src="../../bananas.chunk.js"></script><script type="text/javascript" src="../../${entryName}.chunk.js"></script>`)
        .length > 0
      })
  );
});
